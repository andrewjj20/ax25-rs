...
use libc::{socket, recvfrom, sendto, close, AF_PACKET, SOCK_RAW, AF_AX25, 
    sockaddr, socklen_t, sockaddr_ll, c_void, c_int, c_char, c_ulong};
...
impl Ax25RawSocket {
    ...
    pub fn receive_frame(&self) -> io::Result<Vec<u8>> {
        ...
        unsafe {
            ...
            len = match recvfrom(self.fd, buf.as_mut_ptr() as *mut c_void, buf.len(), 0, sa_ptr, &mut sa_in_sz) {
...
impl ifreq_union {
    fn ifindex(&self) -> c_int {
        c_int::from_be(((self.data[0] as c_int) << 24) + ((self.data[1] as c_int) << 16)
                       + ((self.data[2] as c_int) << 8) + ((self.data[3] as c_int)))
    }
    ...
    fn ax25_address(&self) -> Option<String> {
        let mut addr_utf8: Vec<u8> = self.data[2..8].iter()
            .rev()
            .map(|&c| c >> 1)
            .skip_while(|&c| c == 0)
            .collect::<Vec<u8>>();
